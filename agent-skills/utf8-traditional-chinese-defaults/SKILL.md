...
4. When using PowerShell to read text files where encoding matters, use `Get-Content -Encoding UTF8`.
5. When using PowerShell commands that write text files, specify UTF-8 explicitly, such as `Set-Content -Encoding UTF8`, `Add-Content -Encoding UTF8`, `Out-File -Encoding UTF8`, or `Export-Csv -Encoding UTF8`.
6. If output looks garbled, first determine whether the issue is terminal display, incorrect read encoding, or damaged file contents before rewriting the file.
7. After document changes, verify the relevant file snippets, README indexes, or rendered text when practical.
